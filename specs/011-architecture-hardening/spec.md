...
- **FR-006a**: Every tool description MUST end with a usage example of the form `Example: {...}` — a minimal valid JSON argument object for that tool — so an agent can call it correctly without a schema round-trip (Constitution I: AI-native descriptions). Zero-argument tools use `Example: {} (no arguments).`
