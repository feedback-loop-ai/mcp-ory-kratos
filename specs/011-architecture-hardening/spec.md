...
- **Invocation Trace**: the log entries for one tool call — start (`tool`, `correlationId`) and completion (`tool`, `correlationId`, `durationMs` on completed/failed, plus `error.code`/`error.message` on failure; a cancelled invocation logs `Tool cancelled by user` without `durationMs`). Exactly one completion entry per invocation.
