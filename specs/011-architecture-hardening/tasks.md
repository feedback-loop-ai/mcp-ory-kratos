...
**Retrospective note**: This task list was generated after the implementation landed. Every task that is complete is marked `[x]` with the commit that delivered it: `976cd06` (main refactor), `e9bfcd0` (registration-level confirmation enforcement), `c55ff77` (spec/data-model/quickstart sync), `bb7e40c` (version bump to 0.3.0), `88ca6ec` (constitution PATCH 1.1.1), `4981def` (no-sessions idempotency + e2e), `9e2fdd0` (no explicit `any`), `34db861` (analyze round-1 remediation), and `5b08680` (FR-006a completion + Ajv-validated examples). Items left `[ ]` are listed again under "Open follow-ups".
...
- [x] T006 Extend `Config` in `src/config.ts`: `TOOLSETS`/`Toolset`, `toolsets`, `readOnly`, `confirmDestructive` (default true), `allowCredentialExposure`, `maxScanPages` (default 20, max 1000); `parseToolsets` fails fast listing valid names; `KRATOS_CUSTOM_HEADERS` must be a JSON object of strings; `loadConfig(env = process.env)` injectable, single `ConfigSchema.parse`; empty/unset/`all` `KRATOS_TOOLSETS` = every toolset and booleans accept `1/true/yes/on` (FR-007a); `KRATOS_MAX_SCAN_PAGES` bounded 1–1000 (FR-003a) (FR-003, FR-003a, FR-007, FR-007a, FR-008, FR-009, FR-010, FR-027, D10) (commit 976cd06)
- [x] T007 [P] Create `src/kratos/pagination.ts`: `extractPageToken`/`nextPageTokenOf` (parse `rel="next"` `page_token` from the Axios `link` header), `scanPages(fetchPage, {maxPages, startToken})` returning `{items, pagesScanned, truncated, nextPageToken}` with strictly sequential fetches and no throttling, and `inTimeRange`; `truncated` true only when the cap stopped a scan with pages remaining and always paired with a resume token (FR-002b) (FR-001, FR-002, FR-002b, FR-003, FR-004a, D6) (commit 976cd06)
...
- [x] T011 Add shared schema building blocks to `src/schemas/tools.ts`: `PaginationInputSchema` (`pageSize`/`pageToken`), `PaginatedOutputSchema` (`items`, `count`, `nextPageToken?`), `ScanSummary` (`pagesScanned`, `truncated`, `nextPageToken?`), `MaxPagesInputSchema`, `PassthroughObjectSchema`, and an output schema for every tool; `pageSize` bounded 1–100 default 20 (FR-001a) (FR-001, FR-001a, FR-003, FR-021) (commit 976cd06)
- [x] T012 Create `src/tools/define.ts`: `ToolContext`, `ToolDefinition` (name, title, description, toolset, inputSchema, outputSchema, annotations, `confirmMessage?`, `run`), `defineTool` over `McpServer.registerTool` passing full Zod objects (not `.shape`), adding `openWorldHint:false`, correlated start/`Tool completed`/`Tool failed`/`Tool cancelled by user` logging with `correlationId` + `durationMs` + mapped error code (FR-009b) and never a payload field (FR-012a), `mapError` → `isError:true` envelope without `structuredContent`, `structuredContent` when an output schema exists; annotation presets `READ_ONLY`, `CREATE`, `UPDATE_IDEMPOTENT`, `UPDATE`, `DESTRUCTIVE` (FR-006, FR-009b, FR-012a, FR-021, FR-024a, FR-032, D1, D2) (commit 976cd06)
...
- [x] T019 [P] Unit tests for the foundation in `tests/unit/config.test.ts` (toolset parsing + fail-fast, boolean flags, custom-headers validation, env injection), `tests/unit/pagination.test.ts` (Link-header parsing, `scanPages` cap/end-of-collection/resume semantics, `inTimeRange`), `tests/unit/client.test.ts` (`KratosHttpError`, raw-HTTP timeout → `ETIMEDOUT`); config tests cover the FR-007a spellings and the FR-003a bounds, schema/pagination tests the FR-001a `pageSize` bounds (FR-001, FR-001a, FR-002, FR-003, FR-003a, FR-007a, FR-026, FR-027, FR-028) (commit 976cd06)
...
- [x] T020 [P] [US1] Unit tests in `tests/unit/session-tools.test.ts` via the harness: `kratos_list_sessions` plain page returns `items/count/nextPageToken`; filtered scan walks 100-item upstream pages independent of `pageSize`, stops at `pageSize` matches with a resume cursor, reports `truncated:true` only when `maxPages` stopped the scan; `kratos_list_identity_sessions` cursor; `pageSize`/`pageToken` replace `limit`; `count` equals returned matches and `truncated` implies a cursor (FR-002b) (FR-001, FR-002, FR-002b, FR-004a, FR-005, SC-001, SC-002) (commit 976cd06)
- [x] T021 [P] [US1] Unit tests in `tests/unit/analytics-tools.test.ts` and `tests/unit/analytics.test.ts`: `maxPages` default from config and per-call override, `pagesScanned`/`truncated`/`nextPageToken` in output, device expansion omitted when `includeDevices:false`, pure aggregation helpers; `maxPages` out of 1–1000 rejected by validation (FR-003a) (FR-003, FR-003a, FR-004, FR-004a) (commit 976cd06)
...
- [x] T023 [US1] Rewrite `src/tools/session.ts` via `defineTool`: `pageSize`/`pageToken` on `kratos_list_sessions` (drop `limit`, breaking change) and `kratos_list_identity_sessions`, `nextPageTokenOf` on plain pages, `scanFilteredSessions` at `SDK_MAX_PAGE_SIZE=100` with `maxPages ?? config.maxScanPages`, `pagesScanned`, `truncated` (cap-only), resume `nextPageToken` whenever more upstream pages remain; description tells the agent to raise `maxPages` or resume and states the 100/page scan size (FR-002a) (FR-001, FR-002, FR-002a, FR-004a, FR-005, SC-001, SC-002) (commit 976cd06)
- [x] T024 [P] [US1] Rewrite `src/tools/analytics.ts` via `defineTool` + `scanPages` (250/page): `maxPages` input, `ScanSummary` in output, `expand:["devices"]` only when `includeDevices`, `inTimeRange` filtering; descriptions state the 250/page scan size (FR-002a) (FR-002a, FR-003, FR-004, FR-004a) (commit 976cd06)
...
- [x] T030 [US2] Write action-specific `confirmMessage` prompts for all eight destructive tools (e.g. `Set identity ${id} to ${state} and revoke all its sessions?`, credential target `type (identifier)`) naming the action, every target identifier and the user-visible consequence (FR-009a) in `src/tools/identity.ts` and `src/tools/session.ts`; document confirmation, `KRATOS_CONFIRM_DESTRUCTIVE`, and `KRATOS_READ_ONLY` in the server `instructions` in `src/server.ts` (FR-009, FR-009a, FR-022) (commits 976cd06, e9bfcd0)
...
- [x] T033 [P] [US3] Unit tests in `tests/unit/identity-tools.test.ts` via the harness: `kratos_get_identity` with `includeCredential` list forwards the list and redacts; deprecated `includeCredentials:true` maps to all types; explicit list wins when both are given; `kratos_list_identities` redacts each item; `kratos_get_identity_by_external_id` never requests credentials and so returns none (FR-010a); `KRATOS_ALLOW_CREDENTIAL_EXPOSURE=1` returns raw config (FR-010, FR-010a, FR-011, SC-005) (commit 976cd06)
...
- [x] T035 [US3] Apply `redactCredentials(identity, config.allowCredentialExposure)` in `src/tools/identity.ts` to `kratos_get_identity` and every `kratos_list_identities` item — the only two tools that accept `includeCredential`; `kratos_get_identity_by_external_id` exposes no credential parameter and requests none, so it needs no redaction (FR-010a); resolve `includeCredential ?? (includeCredentials ? ALL_CREDENTIAL_TYPES : undefined)`; document redaction in tool descriptions and server `instructions` (FR-010, FR-010a, FR-011, FR-022, SC-005, D5) (commit 976cd06)
...
- [x] T037 [P] [US4] Unit tests in `tests/unit/batch-patch-identities.test.ts` (ported to the harness): each batch item accepts the full identity import body; results use the FR-016a shape (`{results:[{action, identity, patchId, error}], summary:{total, succeeded, failed}}`); a batch with any failed item logs one `Batch patch had failures` warning carrying `tool`, `correlationId`, `failed` via the injected `log` (FR-016, FR-016a, FR-020a, FR-024a) (commit 976cd06)
- [x] T038 [P] [US4] Unit tests in `tests/unit/identity-external-id.test.ts` (ported to the harness): `kratos_get_identity_by_external_id` structured output, NOT_FOUND envelope, and that no credential data is returned because none is requested (FR-010a) (FR-010, FR-010a, FR-021) (commit 976cd06)
...
- [x] T043 [US4] Rewrite `src/tools/identity.ts` via `defineTool`: `toCreateIdentityBody`/`toUpdateIdentityBody` mapping the import body; filters forwarded on `listIdentities`; `identifier` forwarded on `deleteIdentityCredentials`; new `kratos_set_identity_state` (patch `state`, optional `deleteIdentitySessions`); new `kratos_list_identity_schemas` and `kratos_get_identity_schema`; batch results use the FR-016a shape (breaking, 0.3.0); last-write-wins (no read-before-write, JSON Patch `test` forwarded unchanged); `kratos_update_identity` description warns that omitted metadata is cleared (FR-014, FR-015, FR-016, FR-016a, FR-017, FR-018, FR-020, FR-020a) (commit 976cd06)
...
**Checkpoint**: CI reports a real coverage figure (baseline 89.75% statements / 78.91% branches / 89.36% functions / 90.82% lines, 16 files / 224 tests — plan.md D14), audit is clean, integration job passes against Kratos v26.2.0 (SC-006, SC-007, SC-008)
...
- [x] T056 [US6] Verify zero `try/catch`, `log.info`/`log.error`, `content:[`, `server.tool`/`registerTool` in `src/tools/identity.ts`, `src/tools/session.ts`, `src/tools/courier.ts`, `src/tools/recovery.ts`, `src/tools/health.ts`, `src/tools/analytics.ts` (the single `log.warn` for an all-failed batch in `identity.ts` is the spec-required warning via the injected `RunContext.log`) — measured on the branch: `try {` count is 0 in every tool file except `define.ts` ; the harness log capture confirms tool files emit no payload fields (FR-012a) (FR-012a, FR-024a, FR-032, US6 acceptance 2) (commit 976cd06)
- [x] T057 [US6] Verify SC-009: `src/` total 3088 lines on the branch vs 3.5k on `main` (tool files 1343 lines vs 2220 for the same six files + `index.ts` on `main`) while adding `kratos_set_identity_state`, `kratos_list_identity_schemas`, `kratos_get_identity_schema` (SC-009) (commit 976cd06)
...
- [x] T059 [P] Update `README.md`: new env vars (`KRATOS_TOOLSETS`, `KRATOS_READ_ONLY`, `KRATOS_CONFIRM_DESTRUCTIVE`, `KRATOS_ALLOW_CREDENTIAL_EXPOSURE`, `KRATOS_MAX_SCAN_PAGES`), 27-tool table with Kind column and the three new tools, Behaviour section (confirmation, redaction, pagination/scan cap), "Breaking changes in 0.3.0" section (`limit` → `pageSize`/`pageToken`, `includeCredentials` deprecated + redaction default, batch result shape `{action, identity, patchId, error}` + `summary.total` per FR-016a, destructive tools may return `{cancelled:true}`, MCP version = package version, Node ≥ 20), local integration via `docker compose up -d --wait` (FR-005, FR-016a, FR-022, FR-033, SC-007) (commit 976cd06)
...
- [x] T070 [P] Add the "invocation trace (FR-024a)" suite to `tests/unit/server.test.ts` on top of log capture in `tests/unit/harness.ts`: every invocation yields one start entry and exactly one `Tool completed` / `Tool failed` / `Tool cancelled by user` entry, each carrying `tool`, `correlationId`, `durationMs` (failure adds `error.code`/`error.message`); and no log entry ever contains identity traits or credential configuration (FR-012a) (FR-009b, FR-012a, FR-024a) (commit 34db861)
- [x] T071 [P] Add `pageToken` to `MaxPagesInputSchema` in `src/schemas/tools.ts` and pass it as `scanPages` `startToken` in `src/tools/analytics.ts` so a truncated `kratos_session_analytics` / `kratos_credential_analytics` result can be resumed from its `nextPageToken`; descriptions say so; tests in `tests/unit/analytics-tools.test.ts` (FR-003, US1 acceptance 4) (commit 34db861)
- [x] T072 [P] Replace generic passthrough output schemas on destructive tools with their real shapes in `src/schemas/tools.ts`: `MutationResultSchema` for `kratos_delete_identity`, `kratos_delete_identity_credential`, `kratos_disable_session`; `DeleteIdentitySessionsOutputSchema` (`success`, `message`, `sessionsExisted`) for `kratos_delete_identity_sessions` (returned from `src/tools/session.ts`); `SetIdentityStateOutputSchema` (`id`, `state?`, `sessionsRevoked`, passthrough) for `kratos_set_identity_state`; `SessionSummarySchema` for `kratos_extend_session` — each then widened by `withCancellation` (FR-017a, FR-021, Constitution I) (commit 34db861)
- [x] T073 [P] End every tool description in `src/tools/identity.ts`, `session.ts`, `courier.ts`, `recovery.ts`, `health.ts`, `analytics.ts` with a usage example `Example: {...}` — a minimal valid argument object (FR-006a, Constitution I) (commit 34db861; four identity tools completed and FR-006a enforced by a `tests/unit/server.test.ts` assertion that every example parses and satisfies its own inputSchema via Ajv — commit 5b08680)
- [x] T074 [P] Make the `kratos_extend_session` confirmation prompt in `src/tools/session.ts` state its consequence — `Extend session <id> beyond its current expiry? This widens the user's access window.` — and repeat it in the description (FR-009a) (commit 34db861)
...
- **Polish (Phase 9)**: T059–T062, T064 parallel; T063 after T062; T065 last of the done items; T066 after merge; T067 independent; T068 after T067; T069–T074 independent of each other (T072 touches `src/schemas/tools.ts` after T042)
...
| FR-001a `pageSize` 1–100 default 20, `pageToken` optional, validated before upstream | T011, T019 |
| FR-002 filtered session scan (100/page, cap, `truncated`, resume) | T007, T019, T020, T023 |
| FR-002a scanner page sizes (100 sessions / 250 analytics) stated in descriptions | T023, T024 |
| FR-002b `count` = returned matches; `truncated` ⇒ resume cursor; false at end-of-collection on last page | T007, T020 |
| FR-003 analytics `maxPages`, server-wide default, scan summary, `pageToken` resume | T006, T007, T011, T019, T021, T024, T071 |
| FR-003a `maxPages` / `KRATOS_MAX_SCAN_PAGES` 1–1000 default 20; validation / startup failure | T006, T019, T021 |
...
| FR-009a prompt names action + every target + consequence; one boolean `confirm`; accept-only proceeds | T014, T030, T074 |
| FR-009b fixed `Cancelled by user` message; `Tool cancelled by user` third completion outcome | T012, T014, T027, T070 |
| FR-010 redaction marker for secret-bearing credential config | T006, T010, T032, T033, T035, T038 |
| FR-010a redaction applied to `kratos_get_identity` and `kratos_list_identities` items only; external-ID lookup requests no credentials | T033, T035, T038 |
...
| FR-012a logs never carry traits, credential config, recovery secrets, tokens or bodies | T012, T056, T070 |
...
| FR-016a batch result shape `{results:[{action, identity, patchId, error}], summary}` (breaking, 0.3.0, documented) | T037, T043, T059 |
| FR-017 `kratos_set_identity_state` with session revocation | T036, T041, T042, T043 |
| FR-017a revocation idempotent on 404/400 "no sessions"; `sessionsRevoked` / `sessionsExisted` reported | T068, T072 |
...
| FR-021 output schemas + `structuredContent`, error envelope kept; destructive tools declare real shapes widened for cancellation | T011, T012, T014, T038, T061, T072 |
...
| FR-024a one start + one completion log entry with correlationId/durationMs/error code | T012, T037, T056, T070 |
...
Every FR — FR-001…FR-034 including the lettered sub-requirements FR-001a, FR-002a, FR-002b, FR-003a, FR-004a, FR-006a, FR-007a, FR-009a, FR-009b, FR-010a, FR-012a, FR-016a, FR-017a, FR-020a, FR-024a — and every SC maps to at least one task in the table above; every task cites at least one FR/SC or story acceptance criterion. Verified by grepping each FR identifier against this file.
...
- Test files listed in FR-030 tasks: `identity-tools`, `session-tools`, `courier-tools`, `recovery-tools`, `health-tools`, `analytics-tools`, `server`, `config`, `client`, `pagination`, `batch-patch-identities`, `identity-external-id`, `analytics`, `credential-types`, `schemas`, `sessions-helper` (16 unit files / 224 tests) plus `tests/api/mcp-e2e.test.ts`.
