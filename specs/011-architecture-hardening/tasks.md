...
- [x] T069 Remove the explicit `any` from `src/tools/define.ts`: the `run` wrapper is widened to the SDK callback type via `unknown as ToolCallback<I>` so `src/` carries zero `any` (Constitution VII; plan Constitution Check row VII) (this change)
- [x] T070 [P] Add the "invocation trace (FR-024a)" suite to `tests/unit/server.test.ts` on top of log capture in `tests/unit/harness.ts`: every invocation yields one start entry and exactly one `Tool completed` / `Tool failed` / `Tool cancelled by user` entry, each carrying `tool`, `correlationId`, `durationMs` (failure adds `error.code`/`error.message`); and no log entry ever contains identity traits or credential configuration (FR-012a) (FR-009b, FR-012a, FR-024a) (this change)
- [x] T071 [P] Add `pageToken` to `MaxPagesInputSchema` in `src/schemas/tools.ts` and pass it as `scanPages` `startToken` in `src/tools/analytics.ts` so a truncated `kratos_session_analytics` / `kratos_credential_analytics` result can be resumed from its `nextPageToken`; descriptions say so; tests in `tests/unit/analytics-tools.test.ts` (FR-003, US1 acceptance 4) (this change)
...
- [x] T073 [P] End every tool description in `src/tools/identity.ts`, `session.ts`, `courier.ts`, `recovery.ts`, `health.ts`, `analytics.ts` with a usage example `Example: {...}` — a minimal valid argument object (FR-006a, Constitution I) (this change)
- [x] T074 [P] Make the `kratos_extend_session` confirmation prompt in `src/tools/session.ts` state its consequence — `Extend session <id> beyond its current expiry? This widens the user's access window.` — and repeat it in the description (FR-009a) (this change)
...
| FR-006a every description ends with `Example: {...}` | T073 |
...
- T069–T074 are in the working tree ("this change") and not yet committed; their commit hash replaces "(this change)" once landed.
